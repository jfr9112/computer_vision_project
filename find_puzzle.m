...
% desired orientation
function im_puzzle = find_puzzle(fileName)
    im_rgb = im2double(imread(fileName));

    imshow(im_rgb);
    im_gray = im_rgb(:,:,2); % Green channel
    ...
    [centers, radii, metric] = imfindcircles(~b_im_opened, [min_rad, max_rad], 'Sensitivity', 0.9);
    toc
    ...
    im_cropped = imcrop(im_gray, crop_rect + margin_mat );
